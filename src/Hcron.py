...
class Hcron:
    ...
    DEF_STATE_FILES = ['/var/run/Hcron/state', '/var/run/Hcron/state']
    ...
    def __init__(self):
        ...
        if current_state == 'passive':
            self.write_syslog("cluster=%s state=passive status=ignore cmd=%s" % (
        ...
        self.write_syslog("cluster=%s state=active status=%s dur=%i cmd=%s" % (
        self.config.get('default', 'cluster_name'), 'ok' if rc == 0 else 'fail', runtime, ' '.join(self.args)))
    ...
    def get_state(self):
        ...
        if state not in ('active', 'passive'):
            self.warn("corrupted state file '%s' (content='%s'), using default state (%s)" % (
            self.config.get('default', 'state_file'), state, self.config.get('default', 'default_state')))
    ...
    def open_syslog(self):
        ...
        syslog.openlog(self.progname, syslog.LOG_PID | syslog.LOG_NDELAY,
                       getattr(syslog, self.config.get('default', 'syslog_facility')))
